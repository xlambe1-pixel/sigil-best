...
export default function CollectionMintPage({ slug }: { slug: string }) {
  ...
  const { data: totalMinted } = useReadContract({
    ...
    query: { enabled: !!collection?.contract && collection.contract !== '0x0000000000000000000000000000000000000000' },
  })
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 360px',minHeight:'600px'}}>
        <div style={{padding:'2rem 1.75rem',borderRight:'.5px solid rgba(255,255,255,.06)'}}>
          ...
          <div style={{marginBottom:'1.5rem'}}>
            ...
            <div style={{fontSize:'13px',color:'rgba(255,255,255,.45)',lineHeight:1.9,fontWeight:300}}>{collection.description}</div>
          </div>
          ...
          <div>
            ...
            <div style={{fontFamily:'DM Mono,monospace',fontSize:'11px',color:'#7c6ff7'}}>
              <a href={`https://explorer.ritualfoundation.org/address/${collection.contract}`} target="_blank" style={{color:'#7c6ff7',textDecoration:'none'}}>{collection.contract} ↗</a>
            </div>
          </div>
        </div>
