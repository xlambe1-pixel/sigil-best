...
async function main() {
  ...
  const walletClient = createWalletClient({ account, chain: ritualTestnet, transport: http() });
  const publicClient = createPublicClient({ chain: ritualTestnet, transport: http() });
  ...
  const hash = await walletClient.deployContract({
    ...
    args: ["Sigil Test", "STEST", parseEther("0.05"), 777n, 5n, "ipfs://placeholder/"],
  });
